...
export function JourneyPage() {
  const { result, previousAssessment } = useJourneyStore();
  ...
  if (decision === 'MISSING_INFORMATION') return <>{previous}<div className="result-layout"><MissingInformationCard actions={result.policy_decision.required_actions} /><aside><RiskSignalCard risk={result.risk_signal} /><DecisionSummary journey={result} /></aside></div></>;
  if (decision === 'COMPLEX_REVIEW') return <>{previous}<div className="result-layout"><HumanReviewCard journey={result} /><aside><RiskSignalCard risk={result.risk_signal} /><DecisionSummary journey={result} /></aside></div></>;
  if (decision === 'NOT_SUITABLE') return <>{previous}<NotSuitableSummary /></>;
  return <>{previous}<div className="result-layout"><section className="result-card eligible-card"><span className="eyebrow">GOVERNED NEXT STEP</span><h1>Your journey can move forward</h1><p>Your goal: <strong>{result.customer_goal}</strong></p><div className="path-amount">{formatCurrency(result.requested_amount)}</div><p>{result.next_action}</p><div className="notice">Eligible for the next governed journey step. This is not a loan approval or guarantee.</div></section><aside><RiskSignalCard risk={result.risk_signal} /><DecisionSummary journey={result} /></aside></div></>;
}
